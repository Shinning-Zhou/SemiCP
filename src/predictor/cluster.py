...
class ClusteredPredictor(ClassWisePredictor):
    ...
    Attributes:
        _ratio_clustering (str or float): The ratio of examples in the calibration dataset used to cluster classes.
        _num_clusters (str or int): The number of clusters.
        _split (str): The method to split the dataset into clustering dataset and calibration set.
    """
    ...
    def __init__(self, score_function, model=None, temperature=1, ratio_clustering="auto", num_clusters="auto",
        ...
        self._ratio_clustering = ratio_clustering
    ...
    def _split_data(self, scores, labels, classes_statistics):
        """
        ...
        if self._split == 'proportional':
            # Split dataset along with fraction "frac_clustering"
            num_classes = classes_statistics.shape[0]
            n_k = torch.tensor([int(self._ratio_clustering * classes_statistics[k]) for k in range(num_classes)],
    ...
    def _get_quantile_minimum(self, alpha):
        """
    ...
    def _get_rare_classes(self, labels, alpha, num_classes):
        """
        ...
        """
        thresh = self._get_quantile_minimum(alpha)
    ...
    def _remap_classes(self, labels, rare_classes):
        """
    ...
    def _embed_all_classes(self, scores_all, labels, q=[0.5, 0.6, 0.7, 0.8, 0.9]):
        """
    ...
    def _compute_cluster_specific_qhats(self, cluster_assignments, cal_class_scores, cal_true_labels, alpha):
        """
        ...
        cluster_qhats = self._compute_class_specific_qhats(cal_class_scores, cal_true_clusters, num_clusters, alpha)
    ...
    def _compute_class_specific_qhats(self, cal_class_scores, cal_true_clusters, num_clusters, alpha):
        """
